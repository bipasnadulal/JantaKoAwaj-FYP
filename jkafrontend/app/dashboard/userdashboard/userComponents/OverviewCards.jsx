...
//   const cardData = [
//     { title: "Total Complaints", count: data.total_complaints, icon: icons.complaints, color: "blue" },
//     { title: "Votes Cast", count: data.votes_cast, icon: icons.votes, color: "purple" },
//     { title: "Resolved Issues", count: data.resolved_issues, icon: icons.resolved, color: "green" },
//   ];
...
//         <div key={index} className={`${getBgColor(card.color)} p-5 rounded-lg shadow-sm flex items-center gap-4`}>
    ...
    <div className="grid md:grid-cols-3 gap-6 mb-8">
      {cardData.map((card, index) => (
        <div
          ...
          onClick={() => setActiveTab(card.tab)}  // <-- update activeTab instead of router.push
        >
          <div className="text-3xl">{card.icon}</div>
          <div>
            <h4 className="text-sm font-medium">{card.title}</h4>
