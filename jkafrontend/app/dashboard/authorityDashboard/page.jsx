...
//                 Welcome {authority?.name}
//               </h2>
//               <p className="text-sm text-blue-100">
//                 Handle assigned complaints with transparency.
//               </p>
//             </div>
//             <AuthorityOverviewCards authority={authority} />
//           </>
//         )}

//         {activeTab === "complaints" && <AuthorityComplaintsTable />}
//         {activeTab === "complaints_status" && <ComplaintsList />}
...
export default function AuthorityDashboard() {
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [complaintStatus, setComplaintStatus] = useState('all'); // status filter for ComplaintsList
  const router = useRouter();
